...
Два типа пауз с общей логикой цикла:

| Параметр | Короткая пауза | Длинная пауза |
|---|---|---|
| Интервал | настраивается | каждые N коротких пауз (N настраивается) |
...
- Показывается как единый floating-баннер на активном экране (`NSScreen.main`)
...
**Проверка обновлений каталога**: при запуске приложения и далее по фоновому таймеру раз в сутки; фактическое автообновление выполняется не чаще, чем раз в 7 дней (при включённых `stereogramsEnabled` и `stereogramsAutoUpdate`).
...
● EyeHeaven
─────────────────
Короткий перерыв: через X мин
Длинный перерыв: через Y мин
─────────────────
Начать сейчас: Короткий перерыв
Начать сейчас: Длинный перерыв
─────────────────
⏸ Пауза / ▶ Возобновить
─────────────────
Настройки...
...
Длинная пауза
  ├── Каждые N коротких пауз (дефолт: 3)
  ├── Расчётный интервал (read-only, зависит от интервала короткой паузы и N)
...
- [x] Heartbeat: анонимный пинг раз в сутки
- [x] Локализация: полировка автоперевода
- [ ] Нотаризация и .dmg для GitHub Releases

**Phase 7.5 — Между 7 и 8**
...
3. **Phase 7.5 (между 7 и 8)**: реализовать проверку обновлений приложения
  - Onboarding при первом запуске
  - Проверка обновлений приложения

4. **Phase 8**: реализовать Meeting Detection
  - `MeetingDetector.swift` (process + windows + mic status)
  - Интеграция в `BreakScheduler` через `meetingDelay`

5. **Перед релизом**: вернуть автообновление каталога после локального тестового режима
