...
### XcodeBuildMCP (подключён глобально, имя `xcode`)
Предпочитать MCP-инструменты вместо сырого `xcodebuild` через Bash:
- `xcode_build` — сборка проекта, возвращает структурированные ошибки
- `xcode_test` — запуск тестов
- `xcode_list_simulators` / `xcode_boot_simulator` — управление симулятором

### SwiftFormat (hook)
Автоматически форматирует каждый `.swift` файл после Write/Edit. Настроен в `.claude/settings.json`. Ничего делать не нужно.

### Запрещено агенту
- Редактировать `.xcodeproj` / `.pbxproj` — файлы подхватываются автоматически (PBXFileSystemSynchronizedRootGroup).
- Редактировать `.entitlements` файлы вручную.

---

## Команды сборки

**Обязательно**: после каждого изменения кода запускать Debug-сборку через XcodeBuildMCP (`xcode_build`) или Bash и убеждаться что нет ошибок компилятора перед тем как сообщать пользователю о готовности.
